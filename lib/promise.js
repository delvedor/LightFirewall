...
function addAttempt (LightFirewallDB, ip) {
  let promise = new Promise((resolve, reject) => {
    LightFirewallDB.get(ip, (getErr, client) => {
      if (getErr && !getErr.notFound) reject(getErr)
  ...
  let promise = new Promise((resolve, reject) => {
    LightFirewallDB.get(ip, (getErr, client) => {
      ...
      // 2)
        ...
        addTimeout(LightFirewallDB, ip)
          .then(() => {
            return removeAttempts(LightFirewallDB, ip)
          })
          .then(() => {
            resolve(true)
          })
      ...
      // 3)
        ...
        if (!isTimeout) removeTimeout(LightFirewallDB, ip)
        resolve(isTimeout)
      // 4)
      } else {
        resolve(false)
